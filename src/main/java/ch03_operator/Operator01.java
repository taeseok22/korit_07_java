...
/*
    Camel case -> Java 상에서의 변수 표기법(JS 에서도 쓴다)
    Snake case -> 얘는 뭐 Java 에서 쓸 일이 없다. 근데 전체 상수 사용시에 비슷한 표기법이 적용된다.
    Pascal case -> 첫 번째 문자가 대문자고 나머지는 소문자로 가다가 두 번째 단어의 첫 번째 문자는 또 대문자
        ex) 한 단어 짜리 : Operator01
        ex) 복수 단어 : OperatorTestResult      -> Pascal Case 는 클래스 명명 규칙에 해당한다.
...
public class Operator01 {
    public static void main(String[] args) {
        ...
        int a = 5;      // int 는 정수
        int b = 2;      // 변수 선언 및 초기화
        double result = (double) a / b; // (double)는 강제 '형변환' 개념입니다.
        System.out.println(a/b);        // 근데 2가 나온다
        System.out.println(result);     // 얘는 2.5가 나온다
        ...
        System.out.println(j);          // 10
        // 변수명-- : 코드를 실행시킨 후에 j에 1을 뺌
        // --변수명 : 코드를 실행시키기 전에 j에 1을 뺌

...
//        teacher1.setSchool("코리아아이티");
//        System.out.println(teacher1);

        boolean result1 = teacher1.equals(teacher2);
        System.out.println(result1);    // true
        // 이상의 코드에서 중요한 점은 재정의 전과 후가 결과값이 다르단거다.
        // Teacher에서 .equals()를 재정의하기 전에는 false가 떴었다.
        // 그때의 경우는 teacher1 과 teacher2의 주소지까지를 확인해
        // 같은지 아닌지의 여부를 판단했다면
        // 재정의 이후에는 field에 입력된 데이터가 동일한지만을 체크하기 때문이다.

        Class tClass = teacher1.getClass(); // 지난 시간에 하긴 했다.
        ...
        // 우린 어느 수준까지 그냥 작성할지 변수에 대입할지를 조직에 따라
        // 우리 성향에 따라 결정해야한다.

        Field[] fields = tClass.getDeclaredFields();
//        System.out.println(fields); // 주소지 출력댐
...
//        System.out.println(teacher3 instanceof ch16_objects.Teacher);   // 오류 발생
        // teacher3의 자료형은 ch16_objects.Teacher지
